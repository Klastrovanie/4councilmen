...
interface RunSnapshot {
  results: RoundResult[];
  intentTriage: IntentTriage | null;
  routeBlock: { message: string; triage?: IntentTriage | null } | null;
  finalConclusion: string | null;
...
const THEME_CLASSES: Record<Theme, {
  ...
  'dark-classic': {
    ...
    text: 'text-[#e0e4f8]',
    textMuted: 'text-[#aab4e8]',
    textFaint: 'text-[#7f8ac4]',
    card: 'bg-[#1a1e38]',
  ...
  monokai: {
    ...
    text: 'text-[#f8f8f2]',
    textMuted: 'text-[#cfc6a0]',
    textFaint: 'text-[#9d9678]',
    card: 'bg-[#1e1f1a]',
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .decision-brief.state-singularity { border: 2px solid #16a34a; background: #f0fdf4; color: #14532d; }
    .decision-brief.state-partial, .decision-brief.state-dominant, .decision-brief.state-none { border: 2px solid #f59e0b; background: #fffbeb; color: #78350f; }
    .decision-brief.state-selected { border: 2px solid #9ca3af; background: #f3f4f6; color: #111827; }
    .decision-brief h1, .decision-brief h2, .decision-brief h3, .decision-brief p, .decision-brief div, .decision-brief strong { color: inherit; }
    .brief-title { font-size: 16pt; font-weight: 800; margin-bottom: 2mm; }
    ...
    .brief-item { border-left: 3px solid currentColor; padding-left: 2mm; }
    .brief-item span { display:block; color: inherit; opacity: .72; font-size: 7.5pt; letter-spacing: .08em; text-transform: uppercase; }
    ...
    .consensus.state-singularity { border: 2px solid #16a34a; background: #f0fdf4; color: #14532d; }
    .consensus.state-partial, .consensus.state-dominant, .consensus.state-none { border: 2px solid #f59e0b; background: #fffbeb; color: #78350f; }
    .consensus.state-selected { border: 2px solid #9ca3af; background: #f3f4f6; color: #111827; }
    .consensus, .consensus p, .consensus strong, .consensus div, .consensus .metrics span, .consensus .metrics strong { color: inherit; }
...
<body>
  ...
  <section>
    ...
    <div class="box settings">
      ...
      <div><span>Judge</span>${providerMode === 'all-local' ? 'Local LLM (fixed)' : 'Grok (fixed)'}</div>
...
const DecisionBriefCard: React.FC<DecisionBriefCardProps> = ({ result, finalConclusion, firstSingRound, tc }) => {
  ...
  const palette = decisionBriefPalette(result.convergenceState);
  return (
    <div className={`${palette.box} border-2 rounded-xl p-4 mb-3`}>
      <div className={`text-[10px] font-semibold uppercase tracking-wider ${palette.label} flex items-center gap-1.5 mb-2`}>
        {result.convergenceState === 'singularity' ? <Phone size={11} /> : <PhoneOff size={11} />} Final decision brief
      </div>
      <div className={`text-sm font-semibold ${palette.heading} mb-1`}>
        {buildDecisionTitle(result)} · Signal {result.ratioSignal}
        {firstSingRound && <span className={`text-[11px] font-normal ${palette.muted}`}> · selected round {firstSingRound}</span>}
      </div>
      <p className={`text-[12px] leading-relaxed ${palette.body} mb-2`}>
        <strong className={palette.strong}>Recommended action:</strong> {recommendedAction(result, finalConclusion)}
      </p>
      <p className={`text-[11px] leading-relaxed ${palette.body} mb-2`}>
        <strong className={palette.strong}>Executive rationale:</strong> {summary.oneLine}
      </p>
      {result.whyThisState && (
        <p className={`text-[11px] leading-relaxed ${palette.body} mb-2`}>
          <strong className={palette.strong}>Why:</strong> {result.whyThisState}
        </p>
      )}
      <div className={`grid grid-cols-2 gap-2 text-[10px] ${palette.muted}`}>
        <div><strong className={palette.strong}>State:</strong> {convergenceLabel(result.convergenceState)}</div>
        <div><strong className={palette.strong}>Internal ratio:</strong> {result.ratio.toFixed(4)}</div>
        {result.coalitionAgents.length > 0 && <div className="col-span-2"><strong className={palette.strong}>Coalition:</strong> {result.coalitionAgents.join(', ')}</div>}
        {(result.dissentingAgents.length > 0 || result.weakestLink) && <div className="col-span-2"><strong className={palette.strong}>Dissent / weakest link:</strong> {result.dissentingAgents.length ? result.dissentingAgents.join(', ') : result.weakestLink}</div>}
      </div>
    </div>
  );
};
...
function convergenceBadgeClass(state: ConvergenceState, tc: typeof THEME_CLASSES[Theme]): string {
  if (state === 'singularity') return 'border-green-400 text-green-500 bg-transparent';
  if (state === 'partial_convergence' || state === 'dominant_compatible_proposal' || state === 'no_singularity') return 'border-amber-400 text-amber-500 bg-transparent';
  return 'border-gray-400 text-gray-500 bg-transparent';
}

function convergencePanelClass(state: ConvergenceState, tc: typeof THEME_CLASSES[Theme]): string {
  // Keep the panel background theme-native. A light amber/green background makes
  // dark-theme text nearly invisible, so state is communicated by border + label colour.
  switch (state) {
    case 'singularity':
      return `${tc.card} border-green-400`;
    case 'partial_convergence':
    case 'dominant_compatible_proposal':
    case 'no_singularity':
      return `${tc.card} border-amber-400`;
    default:
      return `${tc.card} border-gray-400`;
  }
}
...
function decisionBriefPalette(state: ConvergenceState): {
  box: string;
  label: string;
  heading: string;
  body: string;
  ...
  // Semantic status colours are intentionally independent from the app theme.
  // Green means full singularity. Amber means no full singularity / review needed.
  // The background and text colours are paired here to guarantee contrast in every theme.
  if (state === 'singularity') {
    return {
      box: 'bg-[#ecfdf5] border-[#22c55e]',
      label: 'text-[#166534]',
      heading: 'text-[#052e16]',
      body: 'text-[#14532d]',
      muted: 'text-[#166534]',
...
function selectedPanelClass(tc: typeof THEME_CLASSES[Theme]): string {
  return `${tc.card} border-gray-400`;
}
...
const FourCM: React.FC = () => {
  ...
  // The scenario/custom tab that owns the current run result.
  // This prevents old results from being displayed after the user selects another scenario.
  const [runOwnerKey, setRunOwnerKey] = useState<string | null>(null);
  // Results are cached per sidebar scenario/custom tab. If the user runs scenario #2,
  // that result is shown only when scenario #2 is selected again; other scenarios do
  // not inherit or display stale results.
  const [runSnapshots, setRunSnapshots] = useState<Record<string, RunSnapshot>>({});
  const runSnapshotsRef = useRef<Record<string, RunSnapshot>>({});
  ...
  useEffect(() => {
    runSnapshotsRef.current = runSnapshots;
  }, [runSnapshots]);
  useEffect(() => {
  ...
  const applyRunSnapshot = useCallback((ownerKey: string, snap?: RunSnapshot | null) => {
    if (snap) {
      setResults(snap.results || []);
      setStreamingText({});
      setIntentTriage(snap.intentTriage || null);
      setRouteBlock(snap.routeBlock || null);
      setFinalConclusion(snap.finalConclusion || null);
      setFirstSingRound(snap.firstSingRound ?? null);
      setRoundIdx(snap.roundIdx || snap.results?.length || 0);
      setRunOwnerKey(ownerKey);
      return;
    }
    setResults([]);
    setStreamingText({});
  ...
  const clearRunOutput = useCallback((ownerKey: string | null = null, removeCached = false) => {
    if (ownerKey && removeCached) {
    ...
    setFinalConclusion(null);
    setFirstSingRound(null);
    setRunOwnerKey(ownerKey && !removeCached ? ownerKey : null);
    setRoundIdx(0);
  }, []);
  ...
  const handleSelectScenario = useCallback((scenario: Scenario) => {
    ...
    setIsRunning(false);
    const nextKey = `scenario:${scenario.id}`;
    setSelectedScenario(scenario);
    setIsCustom(false);
    applyRunSnapshot(nextKey, runSnapshotsRef.current[nextKey] || null);
  }, [applyRunSnapshot, isRunning]);

  const handleCustomMode = useCallback(() => {
    if (isRunning) abortRef.current?.abort();
    setIsRunning(false);
    setIsCustom(true);
    applyRunSnapshot('__custom__', runSnapshotsRef.current['__custom__'] || null);
  }, [applyRunSnapshot, isRunning]);

  const tc = THEME_CLASSES[theme];
  const T = UI_TEXT[lang];

  const currentViewKey = isCustom ? '__custom__' : (selectedScenario?.id ? `scenario:${selectedScenario.id}` : '__none__');
  const isLiveView = runOwnerKey === currentViewKey;
  const cachedViewSnapshot = runSnapshots[currentViewKey] || null;
  const visibleResults = isLiveView ? results : (cachedViewSnapshot?.results || []);
  const visibleIntentTriage = isLiveView ? intentTriage : (cachedViewSnapshot?.intentTriage || null);
  const visibleRouteBlock = isLiveView ? routeBlock : (cachedViewSnapshot?.routeBlock || null);
  const visibleFinalConclusion = isLiveView ? finalConclusion : (cachedViewSnapshot?.finalConclusion || null);
  const visibleFirstSingRound = isLiveView ? firstSingRound : (cachedViewSnapshot?.firstSingRound ?? null);


  useEffect(() => {
    if (!runOwnerKey) return;
    const hasSomething = results.length > 0 || !!routeBlock || !!intentTriage || !!finalConclusion || firstSingRound !== null;
  ...
  }, [runOwnerKey, results, intentTriage, routeBlock, finalConclusion, firstSingRound, roundIdx]);
  ...
  const handlePDF = useCallback(async () => {
    ...
    try {
      ...
      const reportHtml = buildReportHtml({
        ...
        finalConclusion: visibleFinalConclusion,
        firstSingRound: visibleFirstSingRound,
        agents,
        providerMode,
        grokSearchMode,
        claudeKey,
    ...
    visibleResults, pdfTheme, theme, lang, isCustom, selectedScenario, customQuery, T.custom,
    nRounds, visibleFinalConclusion, visibleFirstSingRound, agents, providerMode, grokSearchMode,
  ...
  const handleRun = async () => {
    ...
    const thisRunOwnerKey = isCustom ? '__custom__' : (selectedScenario?.id ? `scenario:${selectedScenario.id}` : '__none__');
    setIsRunning(true);
    clearRunOutput(thisRunOwnerKey, true);
    setRunOwnerKey(thisRunOwnerKey);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className={`px-5 py-3.5 border-b ${tc.headerBorder} ${tc.header} flex items-center justify-between shrink-0`}>
              ...
              <div className="px-5 pt-4">
                <div className={`border rounded-xl p-3 ${tc.card} ${tc.cardBorder}`}>
                  ...
                  <div className={`text-[11px] leading-relaxed ${tc.textMuted}`}>
                    <span className={`inline-block text-[9px] px-1.5 py-0.5 rounded mr-1.5 border ${visibleIntentTriage.human_review_required ? tc.provGrok : tc.singBadge}`}>
                      {visibleIntentTriage.human_review_required ? 'Human review required' : 'Human review not required'}
                    </span>
                    <span className="mr-1.5">Intent: {visibleIntentTriage.intent_type || 'unknown'}</span>
                    {visibleIntentTriage.gdpr_stage != null && <span className="mr-1.5">· GDPR stage {visibleIntentTriage.gdpr_stage}</span>}
                    {visibleIntentTriage.high_risk && <span className="mr-1.5">· high risk</span>}
                    {visibleIntentTriage.reason && <p className="mt-1">{visibleIntentTriage.reason}</p>}
                    {visibleIntentTriage.suggested_handling && <p className="mt-1 italic">{visibleIntentTriage.suggested_handling}</p>}
                  </div>
                </div>
              </div>
            )}
            ...
            {visibleRouteBlock && visibleResults.length === 0 && (
              <div className="px-5 pt-4">
                <div className={`border rounded-xl p-3 ${tc.card} ${tc.cardBorder}`}>
                  <div className={`text-[10px] font-semibold uppercase tracking-wider ${tc.textFaint} mb-2 flex items-center gap-1.5`}>
                  ...
                  <div className={`text-[11px] leading-relaxed ${tc.textMuted}`}>
                    <p className={`font-semibold ${tc.text}`}>{visibleRouteBlock.message}</p>
                    {visibleRouteBlock.triage?.intent_type && (
                      <p className="mt-1">Intent: {visibleRouteBlock.triage.intent_type}{visibleRouteBlock.triage.gdpr_stage != null ? ` · GDPR stage ${visibleRouteBlock.triage.gdpr_stage}` : ''}</p>
                    )}
                    {visibleRouteBlock.triage?.reason && <p className="mt-1">Reason: {visibleRouteBlock.triage.reason}</p>}
                    {visibleRouteBlock.triage?.suggested_handling && <p className="mt-1 italic">Suggested handling: {visibleRouteBlock.triage.suggested_handling}</p>}
                  </div>
                </div>
              </div>
            )}
            ...
            {visibleResults.length > 0 && (
              <div className="px-5 pt-5 pb-4">
                <div className={`text-[10px] font-semibold uppercase tracking-wider ${tc.textFaint} mb-3`}>
                  Results · {visibleResults.length} round{visibleResults.length > 1 ? 's' : ''}
                </div>

                <DecisionBriefCard
                  result={visibleFirstSingRound ? (visibleResults.find(r => r.round === visibleFirstSingRound) || visibleResults[visibleResults.length - 1]) : visibleResults[visibleResults.length - 1]}
                  finalConclusion={visibleFinalConclusion}
                  firstSingRound={visibleFirstSingRound}
                  tc={tc}
                />
                ...
                {(visibleFinalConclusion || visibleFirstSingRound !== null) && (
                  <div className={`${selectedPanelClass(tc)} border-2 rounded-xl p-4 mt-3`}>
                    <div className={`text-[10px] font-semibold uppercase tracking-wider ${tc.textMuted} flex items-center gap-1.5 mb-2`}>
                      <Phone size={11} />
                      {visibleFirstSingRound
                        ? `The phone rang · first singularity at round ${visibleFirstSingRound}`
                        : 'The phone did not ring'}
                    </div>
                    {visibleFinalConclusion && (
            ...
            {/* Empty state */}
            {visibleResults.length === 0 && !isRunning && !visibleRouteBlock && (
